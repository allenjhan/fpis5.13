object StreamsUnfoldMethodsTest extends App{

  val s1 = Stream.apply(1, 2, 3, 4, 5)
  val s2 = Stream.apply(6, 7, 8, 9, 10)

  println(s1.map(_+1).toList)
  println(s1.take(2).toList)
  println(s1.takeWhile(_%2==1).toList)
  println(Stream.zipWith(s1, s2)(_+_).toList)
  println(s1.zipAll(s2.take(4)).toList)
